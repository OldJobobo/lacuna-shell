...
class QmlContractTests(unittest.TestCase):
    ...
    def test_lacuna_frame_border_option_is_configurable_and_separate_from_shadow(self):
        ...
        self.assertIn("hostBarPopoutBorderGapProvider: function(screen)", bar)
        self.assertIn("function barPopoutBorderGapFor(screen)", bar)
        self.assertIn("var companionEdge = root.portraitCompanionEdge(screen)", bar)
        self.assertIn("if (edge !== root.position && edge !== companionEdge) return null", bar)
        self.assertIn("barPopoutBorderGap: root.barPopoutBorderGapFor(modelData)", bar)
        self.assertIn("barPopoutBorderGap: root.hostBarPopoutBorderGapFor(modelData)", menu_window)
        self.assertIn("property var hostBarPopoutBorderGapProvider: null", menu_window)
        ...
        self.assertIn("readonly property real attachmentGapTop: Math.max(verticalGapMinimum, attachedFlyoutY + borderInset)", border_window)
        self.assertIn("readonly property real attachmentGapBottom: Math.min(verticalGapMaximum, attachedFlyoutY + attachedFlyoutHeight - borderInset)", border_window)
        self.assertIn("property var barPopoutBorderGap: null", border_window)
        self.assertIn("readonly property real topHorizontalLeftEndX", border_window)
        self.assertIn("readonly property real bottomHorizontalRightEndX", border_window)
        self.assertIn('barGapEdge === "top"', border_window)
        self.assertIn('barGapEdge === "bottom"', border_window)
        self.assertIn('barGapEdge === "left"', border_window)
        self.assertIn('barGapEdge === "right"', border_window)
        self.assertIn("function composeVerticalGaps(", border_window)
        self.assertIn("readonly property real rightVerticalSecondUpperEndY", border_window)
        self.assertIn("readonly property real leftVerticalSecondLowerEndY", border_window)
        ...
        self.assertIn("readonly property real attachmentGapTop: Math.max(verticalGapMinimum, attachedOutlineTop + borderInset)", overlay)
        self.assertIn("readonly property real attachmentGapBottom: Math.min(verticalGapMaximum, attachedOutlineBottom - borderInset)", overlay)
        self.assertIn("property var barPopoutBorderGap: null", overlay)
        self.assertIn("readonly property real topHorizontalLeftEndX", overlay)
        self.assertIn("readonly property real bottomHorizontalRightEndX", overlay)
        self.assertIn("function composeVerticalGaps(", overlay)
        self.assertIn("readonly property real rightVerticalSecondUpperEndY", overlay)
        self.assertIn("readonly property real leftVerticalSecondLowerEndY", overlay)
    ...
    def test_lacuna_tray_dispatches_status_notifier_context_menus(self):
        ...
        navigator = read("lacuna.tray/TrayMenuNavigator.qml")
        self.assertIn("TrayMenuNavigator {", qml)
        self.assertIn("id: submenuOpenerComponent", qml)
        self.assertIn("rootChildren: trayMenuOpener.children", qml)
        self.assertIn("model: root.currentChildren", qml)
        self.assertIn("if (root.trayMenuSettling) return", qml)
        self.assertIn("root.enterSubmenu(menuRow.modelData, menuRow.rowText)", qml)
        self.assertIn("property int settleDuration: 250", navigator)
        self.assertIn("function handleMenuChanged(opener)", navigator)
        self.assertIn("function resetForRoot(nextRoot)", navigator)
        self.assertNotIn("menuRow.modelData.display(", qml)
        ...
        open_menu = qml[qml.index("function openTrayMenu"):qml.index("function trayIconSource")]
        native_branch = open_menu[open_menu.index("if (!item.menu)"):open_menu.index("// Publish an empty nested hierarchy")]
        self.assertIn("var anchorWindow = anchorItem.QsWindow.window", native_branch)
        self.assertLess(native_branch.index("root.close()"), native_branch.index("item.display(anchorWindow"))
        self.assertLess(open_menu.index("trayMenuNavigator.resetForRoot(item)"), open_menu.index("activeTrayItem = item"))
    ...
    def test_ambience_host_orders_siblings_and_suppresses_fallback_windows(self):
        ...
        self.assertIn("Number(source.topHorizontalLeftEndX", foreground_border)
        self.assertIn("Number(source.bottomHorizontalRightEndX", foreground_border)
        self.assertIn("PathMove { x: root.topRightStart", foreground_border)
        self.assertIn("PathMove { x: root.bottomLeftStart", foreground_border)
        self.assertIn("Number(source.rightVerticalSecondUpperEndY", foreground_border)
        self.assertIn("Number(source.leftVerticalSecondLowerEndY", foreground_border)
        self.assertIn("joinStyle: ShapePath.MiterJoin", foreground_border)
    ...
    def test_media_player_video_waits_for_high_res_background_stream(self):
        ...
        self.assertIn("readonly property int normalFadeCoverRiseDuration: 220", overlay)
        self.assertIn("readonly property int normalSourceHoldDuration: 80", overlay)
        self.assertIn("readonly property int normalFadeInDuration: 850", overlay)
        self.assertIn("readonly property int normalExitFadeToBlackDuration: 240", overlay)
        self.assertIn("readonly property int normalExitFadeFromBlackDuration: 700", overlay)
        ...
        self.assertIn("function resolvedLocalCoverOpacity(localPlayerReady)", overlay)
        ...
        self.assertIn("opaque until PlayingState confirms presentation started", overlay)
    ...
    def test_media_player_favorites_are_available_in_menu_ui(self):
        ...
        self.assertIn("readonly property bool backgroundModeSelected", tile)
        self.assertIn("foreground: root.backgroundModeSelected ? root.accent : root.foreground", tile)
        ...
        self.assertIn('property string sourceUrl: ""', tile)
        self.assertIn("source: previewPlayerLoader.sourceUrl", tile)
        self.assertIn("function inlineSourceGenerationIsCurrent(player)", tile)
        self.assertIn("function markInlineFrameReady(player)", tile)
        self.assertIn("target: previewOutput.videoSink", tile)
        self.assertIn("renderer.lacunaReady !== true", tile)
        self.assertIn("root.previewPlayer.lacunaReady === true ? 0 : 1", tile)
